...
---@field __call? fun(self: table, o: SMODS.Tag): SMODS.Tag
---@field extend? fun(self: table, o: SMODS.Tag): table Primary method of creating a class. 
---@field check_duplicate_register? fun(self: table, o: SMODS.Tag): table
...
---@field take_ownership? fun(self: table, key: string, obj: table, silent?: boolean): SMODS.Tag Takes control of vanilla objects. Child class must have get_obj for this to function
---@field get_obj? fun(self: table, key: string): table|nil Returns an object if one matches the `key`. 
---@field apply? fun(self: table, tag: Tag, context: table) Defines behaviour when a tag triggers. To trigger, call `Tag:yep()`. See [SMODS.Tag](https://github.com/Steamodded/smods/wiki/SMODS.Tag) wiki for details. 
---@field set_ability? fun(self: table, tag: Tag) Sets up initial ability for the tag. Values should be stored in `tag.ability`. 
