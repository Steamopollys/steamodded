...
---@field send_to_subclasses? fun(self: table|SMODS.Booster, ...: any): string Starting from this class, recusively searches for functions with the given key on all subordinate classes and run all found functions with the given arguments. 
...
---@field inject? fun(self: table|SMODS.Booster) Called during `inject_class`. Injects the object into the game. 
---@field take_ownership? fun(self: table|SMODS.Booster, key: string, obj: table, silent?: boolean): SMODS.Booster Takes control of vanilla objects. Child class must have get_obj for this to function
