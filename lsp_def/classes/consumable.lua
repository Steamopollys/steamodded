...
---@field check_duplicate_key? fun(self: SMODS.Consumable|table): boolean? Ensures objects with duplicate keys will not register. Checked on __call but not take_ownerhsip. For take_ownership, the key must exist. 
...
---@field send_to_subclasses? fun(self: SMODS.Consumable|table, ...: any) Starting from this class, recusively searches for functions with the given key on all subordinate classes and run all found functions with the given arguments. 
...
---@field take_ownership? fun(self: SMODS.Consumable|table, key: string, obj: table, silent?: boolean): nil|SMODS.Consumable Takes control of vanilla objects. Child class must have get_obj for this to function
---@field get_obj? fun(self: SMODS.Consumable|table, key: string): table? Returns an object if one matches the `key`. 
