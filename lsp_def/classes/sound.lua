...
---@field send_to_subclasses? fun(self: table|SMODS.Sound, ...: any): string Starting from this class, recusively searches for functions with the given key on all subordinate classes and run all found functions with the given arguments. 
...
---@field inject? fun(self: table|SMODS.Sound) Called during `inject_class`. Injects the object into the game. 
---@field take_ownership? fun(self: table|SMODS.Sound, key: string, obj: table, silent?: boolean): SMODS.Sound Takes control of vanilla objects. Child class must have get_obj for this to function
