...
---@field send_to_subclasses? fun(self: table|SMODS.Seal, ...: any): string Starting from this class, recusively searches for functions with the given key on all subordinate classes and run all found functions with the given arguments. 
...
---@field inject? fun(self: table|SMODS.Seal) Called during `inject_class`. Injects the object into the game. 
---@field take_ownership? fun(self: table|SMODS.Seal, key: string, obj: table, silent?: boolean): SMODS.Seal Takes control of vanilla objects. Child class must have get_obj for this to function
